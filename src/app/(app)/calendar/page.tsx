...
import { fetchContent, type ContentItem } from '@/lib/api'

const statusColors: Record<string, string> = {
  '💡 Ideas': 'bg-yellow-300',
  '✍️ Copy': 'bg-purple-300',
  '🎨 Diseño': 'bg-pink-300',
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetchContent().then(d => setItems(d)).catch(() => {}),
      fetch('/api/instagram').then(r => r.json()).then(d => { if (d.posts?.length) setIgPosts(d.posts) }).catch(() => {}),
    ]).finally(() => setLoading(false))
  }, [])

  const posts = source === 'notion'
    ? items.map(i => ({ id: i.id, hook: i.hook || i.idea, status: i.status, formato: i.formato, fecha: i.fecha, pilar: i.pilar, assetLink: i.assetLink }))
    : igPosts.map((p: any) => ({ id: p.id, hook: p.hook, status: '✅ Published', formato: p.formato || 'Post', fecha: p.fecha, media_url: p.media_url, permalink: p.permalink }))
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {source === 'notion' ? `Notion · ${items.length} items` : (
              <span className="flex items-center gap-1.5"><Instagram className="w-4 h-4 text-pink-500" /> @kepleragents</span>
            )}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <button onClick={prev} className="p-2 hover:bg-gray-100 rounded-xl transition"><ChevronLeft className="w-5 h-5" /></button>
            <h2 className="text-lg font-semibold capitalize">{monthName} {year}</h2>
            <button onClick={next} className="p-2 hover:bg-gray-100 rounded-xl transition"><ChevronRight className="w-5 h-5" /></button>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
              ...
              return (
                <button key={day} onClick={() => dayPosts.length > 0 && setSelectedDay(day)}
                  className={`aspect-square rounded-xl p-1.5 text-sm transition relative ${isToday ? 'ring-2 ring-indigo-500' : ''} ${dayPosts.length > 0 ? 'hover:bg-gray-50 cursor-pointer' : 'cursor-default'}`}>
              ...
              <div className="space-y-3">
                {selectedPosts.map(p => (
                  <div key={p.id} className="bg-gray-50 rounded-xl p-4">
                    {(p as any).media_url && <img src={(p as any).media_url} alt="" className="w-full h-32 object-cover rounded-lg mb-3" />}
                    <div className="flex items-center gap-2 mb-1">
                      <div className={`w-2 h-2 rounded-full ${statusColors[p.status] || 'bg-blue-300'}`} />
                      <span className="text-xs text-gray-400">{p.formato} · {p.status}</span>
                    </div>
